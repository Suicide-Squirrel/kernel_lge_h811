		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	if (page_old == page_new) {
		...
		retval = touch_i2c_write_byte(ts->client, PAGE_SELECT_REG,
				page_new);

		if (retval < 0) {
			TOUCH_RMIDEV_MSG(
					"Failed to change Page_Select register\n");
			return retval;
		}
	...
	if (retval < 0) {
		TOUCH_RMIDEV_MSG(
				"Failed to write to the register(addr=0x%04x)\n",
				addr);
		return retval;
	}
	...
	if (page_changed) {
		...
		if (retval < 0) {
			TOUCH_RMIDEV_MSG(
					"Failed to restore Page_Select register\n");
			return retval;
		}
	}
...
static int rmidev_reset_device(struct synaptics_ts_data *ts)
{
	...
	for (reg_read_cnt = 1; reg_read_cnt <= 10; reg_read_cnt++) {
		retval = rmidev_i2c_read(ts, ts->f01.dsc.query_base,
				&manufacturer_ID, 1);

		if (retval < 0) {
			TOUCH_RMIDEV_MSG(
					"Failed to read from manufacturer ID register\n");
			return retval;
		}
	...
	retval = rmidev_i2c_write(ts, ts->f01.dsc.control_base + 1,
	...
	if (retval < 0) {
		TOUCH_RMIDEV_MSG(
				"Failed to read from device control register\n");
		return retval;
	}

	device_ctrl |= CONFIGURED;

	retval = rmidev_i2c_write(ts, ts->f01.dsc.control_base,
			&device_ctrl, 1);
...
static int rmidev_irq_enable(struct synaptics_ts_data *ts, bool enable)
{
	...
	if (enable) {
		...
		retval = rmidev_i2c_read(ts, ts->f01.dsc.data_base + 1,
				&intr_status, 1);
		...
		retval = request_threaded_irq(ts->client->irq,
				touch_irq_handler,
...
static irqreturn_t rmidev_sysfs_irq(int irq, void *data)
{
	...
	sysfs_notify(&lge_ts->input_dev->dev.kobj, SYSFS_FOLDER_NAME,
			"attn_state");
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (length) {
		retval = rmidev_i2c_read(ts, address, (u8 *)buf, length);
		...
		char *buf, loff_t pos, size_t count)
{
	...
	if (length) {
		retval = rmidev_i2c_write(ts, address, (u8 *)buf, length);
...
static int rmidev_init_device(struct synaptics_ts_data *ts)
{
	...
	if (retval < 0) {
		...
		retval = gpio_export_link(&(lge_ts->input_dev->dev),
				"attn", ts->pdata->int_pin);
