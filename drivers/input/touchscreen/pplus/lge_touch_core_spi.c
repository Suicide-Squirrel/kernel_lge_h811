...
void send_uevent_lpwg(struct spi_device *spi, int type)
{
	...
	if (type > 0 && type <= VALID_LPWG_UEVENT_SIZE
		...
		send_uevent(&spi->dev, lpwg_uevent[type-1]);
	}
 ...
 * remove ghost event.
 */
static enum filter_return_type bouncing_filter(struct spi_device *spi,
		u32 *report_id_mask)
{
	struct lge_touch_data *ts = spi_get_drvdata(spi);
 ...
 *
 */
static enum filter_return_type grip_filter(struct spi_device *spi,
		u32 *report_id_mask)
{
	struct lge_touch_data *ts = spi_get_drvdata(spi);
...
static enum filter_return_type accuracy_filter(struct spi_device *spi,
		u32 *report_id_mask)
{
	struct lge_touch_data *ts = spi_get_drvdata(spi);
 ...
 */
static enum filter_return_type jitter_filter(struct spi_device *spi,
		u32 *report_id_mask)
{
	struct lge_touch_data *ts = spi_get_drvdata(spi);
 ...
 *
 */
static enum filter_return_type quickcover_filter(struct spi_device *spi,
		u32 *report_id_mask)
{
	struct lge_touch_data *ts = spi_get_drvdata(spi);
...
static int core_filter(struct lge_touch_data *ts)
{
	...
	list_for_each(pos, &ts->filter_head.list) {
		...
		ret &= tmp->filter(ts->spi_device, &tmp_mask);
...
static int report_event(struct lge_touch_data *ts)
{
	...
	u16 new_id = 0;
	u32 new_id_mask = 0;
	//char log_buf[50] = {0};// not used in pplus
	//int rc = 0; // not used in pplus
	u16 prev_id;
	u16 curr_id;
...
static int power_control(struct lge_touch_data *ts, int on_off)
{
	...
	if (atomic_read(&ts->state.power) != on_off) {
		DO_IF(touch_device_func->power(ts->spi_device, on_off) != 0, error);
	...
	if (atomic_read(&ts->state.power) == POWER_OFF)	{
		...
		atomic_set(&ts->state.need_font_downoload, DOWNLOAD_NEED);
		}
...
static int interrupt_control(struct lge_touch_data *ts, int on_off)
{
	...
	if (atomic_read(&ts->state.interrupt) != on_off) {
		...
		if (ts->pdata->role->wake_up_by_touch && !is_probe) {
			...
			enable_irq_wake(ts->spi_device->irq);
		}
	}
...
static void safety_reset(struct lge_touch_data *ts)
{
	...
	case SOFT_RESET:
		TOUCH_I("SOFT_RESET\n");
		DO_IF(touch_device_func->ic_ctrl(ts->spi_device,
...
int touch_ic_init(struct lge_touch_data *ts, int is_error)
{
	...
	DO_IF(touch_device_func->init(ts->spi_device) != 0, error);
...
static void font_download_func(struct work_struct *work_font_download)
{
	struct lge_touch_data *ts = container_of(to_delayed_work(work_font_download),
			struct lge_touch_data, work_font_download);

	TOUCH_TRACE();

	if ( ts->ext_wdata.font_data == NULL )
	{
		TOUCH_E("%s : font download is empty\n", __func__);
		goto cancle_font_download;
		//ts->ext_wdata.font_data = (u8 *)font_example;
	}

	mutex_lock(&ts->pdata->thread_lock);
	TOUCH_I("%s : font download start\n", __func__);
	interrupt_control(ts, INTERRUPT_DISABLE);
	...
	touch_device_func->etw_font_download(ts->spi_device, ts->ext_wdata.font_data);
	atomic_set(&ts->state.need_font_downoload, DOWNLOAD_FINISH);

	interrupt_control(ts, INTERRUPT_ENABLE);
	TOUCH_I("%s : font download end\n", __func__);
	mutex_unlock(&ts->pdata->thread_lock);

cancle_font_download:
	return;
}
...
static void firmware_upgrade_func(struct work_struct *work_upgrade)
{
	...
	ret = touch_device_func->fw_upgrade(ts->spi_device,
...
static void inspection_crack_func(struct work_struct *work_crack)
{
	...
	if (atomic_read(&ts->state.upgrade) != UPGRADE_START) {
		...
		window_crack_check =
			touch_device_func->inspection_crack(ts->spi_device);
...
do_soft_reset:
	...
	if (ts->pdata->panel_id == 0 || ts->pdata->panel_id == 1) {
		touch_swipe_status(ts->spi_device,
				ts->pdata->swipe_stat[1]);
		touch_sleep_status(ts->spi_device,
...
error_in_lpwg:
	...
	if (ts->pdata->panel_id == 1)
		mdelay(10);
	touch_device_func->lpwg(ts_data->spi_device,
...
static void change_ime_drumming_func(struct work_struct *work_ime_drumming)
{
	...
	mutex_lock(&ts->pdata->thread_lock);
	touch_device_func->ime_drumming(ts->spi_device, ime_stat);
	mutex_unlock(&ts->pdata->thread_lock);
...
static void toggle_swipe(struct work_struct *work_swipe)
{
	...
	mutex_lock(&ts->pdata->thread_lock);
	touch_device_func->toggle_swipe(ts->spi_device);
	mutex_unlock(&ts->pdata->thread_lock);
...
void change_thermal_param(struct work_struct *work_thermal)
{
	...
	TOUCH_I("%s: Before changing thermal prarmeters...\n",
			__func__);
	mutex_lock(&ts->pdata->thread_lock);
	touch_device_func->ic_ctrl(ts->spi_device,
 ...
 */
static ssize_t show_platform_data(struct spi_device *spi, char *buf)
{
	struct lge_touch_data *ts = spi_get_drvdata(spi);
...
static ssize_t store_platform_data(struct spi_device *spi,
 ...
 */
static ssize_t store_power_ctrl(struct spi_device *spi,
		const char *buf, size_t count)
{
	struct lge_touch_data *ts = spi_get_drvdata(spi);
	...
	if (!strcmp(string, "reset")) {
		...
		if (value)
			touch_device_func->power(ts->spi_device, POWER_ON);
		else
			touch_device_func->power(ts->spi_device, POWER_OFF);
...
static ssize_t show_ic_rw(struct spi_device *spi, char *buf)
{
	struct lge_touch_data *ts = spi_get_drvdata(spi);
	u32 ret = 0, tmp = 0;
...
static ssize_t store_ic_rw(struct spi_device *spi,
		const char *buf, size_t count)
{
	struct lge_touch_data *ts = spi_get_drvdata(spi);
 ...
 */
static ssize_t show_notify(struct spi_device *spi, char *buf)
{
	struct lge_touch_data *ts = spi_get_drvdata(spi);
	int ret = 0;
...
static ssize_t store_notify(struct spi_device *spi,
		const char *buf, size_t count)
{
	struct lge_touch_data *ts = spi_get_drvdata(spi);
	int code, value;
	...
	if (atomic_read(&ts->state.power) == POWER_ON)
		touch_device_func->notify(ts->spi_device, (u8)code, value);
 ...
 */
static ssize_t store_upgrade(struct spi_device *spi,
		const char *buf, size_t count)
{
	struct lge_touch_data *ts = spi_get_drvdata(spi);
	char path[256] = {0, };
...
static ssize_t show_upgrade(struct spi_device *spi, char *buf)
{
	struct lge_touch_data *ts = spi_get_drvdata(spi);
	int ret = 0;
...
static ssize_t show_lpwg_data(struct spi_device *spi, char *buf)
{
	int i = 0, ret = 0;
	...
	if (touch_device_func->lpwg) {
		...
		touch_device_func->lpwg(spi, LPWG_READ, 0, lpwg_data);
...
static ssize_t store_lpwg_data(struct spi_device *spi,
		const char *buf, size_t count)
{
	struct lge_touch_data *ts = spi_get_drvdata(spi);
	int reply = 0;
	...
	if (reply == 0) {
		...
		if (touch_device_func->lpwg) {
			mutex_lock(&ts->pdata->thread_lock);
			touch_device_func->lpwg(spi, LPWG_REPLY,
					reply, NULL);
			mutex_unlock(&ts->pdata->thread_lock);
		}
	}
 ...
 * 8 : Double Tap Check
 */
static ssize_t store_lpwg_notify(struct spi_device *spi,
		const char *buf, size_t count)
{
	struct lge_touch_data *ts = spi_get_drvdata(spi);
	int type = 0;
 ...
 */
static ssize_t store_keyguard_info(struct spi_device *spi,
		const char *buf, size_t count)
{
	int value;
	struct lge_touch_data *ts = spi_get_drvdata(spi);
...
static ssize_t show_ime_drumming_status(struct spi_device *spi, char *buf)
{
	int ret = 0;
	/* TO DO : */
...
static ssize_t store_ime_drumming_status(struct spi_device *spi,
		const char *buf, size_t count)
{
	int value;
	struct lge_touch_data *ts = spi_get_drvdata(spi);
...
static ssize_t store_quick_cover_status(struct spi_device *spi,
		const char *buf, size_t count)
{
	int value;
	if (sscanf(buf, "%d", &value) <= 0)
...
static ssize_t store_incoming_call(struct spi_device *spi,
		const char *buf, size_t count)
{
	struct lge_touch_data *ts = spi_get_drvdata(spi);
	...
	if (value <= INCOMING_CALL_OFFHOOK) {
		TOUCH_I("%s : %s(%d)\n", __func__,
				...
				call_state);
		mutex_lock(&ts->pdata->thread_lock);
		touch_device_func->incoming_call(ts->spi_device, call_state);
		mutex_unlock(&ts->pdata->thread_lock);
	} else {
		TOUCH_I("%s : %d\n", __func__,
				call_state);
...
static ssize_t show_window_crack_status(struct spi_device *spi, char *buf)
{
	int ret = 0;
...
static ssize_t store_window_crack_status(struct spi_device *spi,
		const char *buf, size_t count)
{
	int value;
	if (sscanf(buf, "%d", &value) <= 0)
...
static ssize_t show_lpwg_all(struct spi_device *spi, char *buf)
{
	struct lge_touch_data *ts = spi_get_drvdata(spi);
	int ret = 0;
...
static ssize_t store_lpwg_all(struct spi_device *spi,
		const char *buf, size_t count)
{
	struct lge_touch_data *ts = spi_get_drvdata(spi);
	int value;
...
static ssize_t store_extwatch_onoff(struct spi_device *spi,
		const char *buf, size_t count)
{
	struct lge_touch_data *ts = spi_get_drvdata(spi);
	enum error_type ret = NO_ERROR;
	...
	ret = touch_device_func->etw_shutdown(ts->spi_device, value);
		...
		const char *buf, size_t count)
{
	...
	ts->ext_wdata.time.disp_waton = value;
	ret = touch_device_func->etw_set_cfg(ts->spi_device, &ts->ext_wdata);
	ret = touch_device_func->etw_onoff(ts->spi_device, ts->ext_wdata.time.disp_waton);
...
static ssize_t show_extwatch_fontdata_query
	(struct spi_device *spi, char *buf)
{
	struct ExtWatchFontDataQuery query;

	query.Font_supported = SURPPORT;
	...
	(struct spi_device *spi, char *buf)
{
	...
	query.vertical_position_supported = NOTSURPPORT;
	query.horizontal_position_supported = SURPPORT;
	...
	(struct spi_device *spi, char *buf)
{
	...
	query.h24_supported = SURPPORT;
	query.AmPm_supported = NOTSURPPORT;

	TOUCH_I("%s %2d %2d\n", __func__, query.h24_supported, query.AmPm_supported);
	...
	(struct spi_device *spi, char *buf)
{
	...
	query.LUT_supported = SURPPORT;
	query.alpha_supported = SURPPORT;
	query.gradation_supported = SURPPORT;
	...
	(struct spi_device *spi, char *buf)
{
	...
	touch_device_func->etw_gettime(ts->spi_device, &ts->ext_wdata);

	ret = snprintf(buf, PAGE_SIZE, "%02d:%02d:%02d \n", ts->ext_wdata.time.rtc_ctst.hour,
		ts->ext_wdata.time.rtc_ctst.min, ts->ext_wdata.time.rtc_ctst.sec);

	return ret;
}

static enum error_type extwatch_dataload(struct spi_device *spi)
{
	struct lge_touch_data *ts = spi_get_drvdata(spi);

	DO_IF(touch_device_func->etw_get_cfg(ts->spi_device, &ts->ext_wdata)!=0, error);
...
static ssize_t store_extwatch_fonteffect_config(struct spi_device *spi,
		const char *buf, size_t count)
{
	struct lge_touch_data *ts = spi_get_drvdata(spi);
	...
	ts->ext_wdata.position.zero_disp = cfg.zero_disp;
	ts->ext_wdata.position.clock_disp_mode = cfg.clock_disp_type;
	ts->ext_wdata.position.bhprd = cfg.blink.blink_type;
	ts->ext_wdata.mode.blink_area.bstartx = cfg.blink.bstartx;
	ts->ext_wdata.mode.blink_area.bendx = cfg.blink.bendx;
	ts->ext_wdata.time.disp_waton = cfg.watchon;
...
static ssize_t store_extwatch_fontproperty_config(struct spi_device *spi,
		const char *buf, size_t count)
{
	struct lge_touch_data *ts = spi_get_drvdata(spi);
	...
	for ( idx = 0; idx < (int)cfg.max_num; idx ++) {
		ts->ext_wdata.mode.lut[idx].b = cfg.LUT[idx].RGB_blue;
		ts->ext_wdata.mode.lut[idx].g = cfg.LUT[idx].RGB_green;
		ts->ext_wdata.mode.lut[idx].r = cfg.LUT[idx].RGB_red;

		len += snprintf(log + len, 256 - len, "%d:%02X%02X%02X ", idx,
			cfg.LUT[idx].RGB_blue, cfg.LUT[idx].RGB_green, cfg.LUT[idx].RGB_red);
	}
...
static ssize_t store_extwatch_fontposition_config(struct spi_device *spi,
		const char *buf, size_t count)
{
	struct lge_touch_data *ts = spi_get_drvdata(spi);
	...
	ts->ext_wdata.mode.watch_area.watendx = cfg.watendx;
	ts->ext_wdata.position.h10x_pos = cfg.h10x_pos;
	ts->ext_wdata.position.h1x_pos = cfg.h1x_pos;
	ts->ext_wdata.position.m10x_pos = cfg.m10x_pos;
	ts->ext_wdata.position.m1x_pos = cfg.m1x_pos;
	ts->ext_wdata.position.clx_pos = cfg.clx_pos;

	TOUCH_I("%s : Watch area [%d , %d] position [%d %d %d %d %d]\n", __func__,
		cfg.watstartx, cfg.watendx,cfg.h10x_pos, cfg.h1x_pos,cfg.clx_pos,
		cfg.m10x_pos, cfg.m1x_pos);

	return count;
}

static ssize_t store_extwatch_timesync_config(struct spi_device *spi,
		const char *buf, size_t count)
{
	struct lge_touch_data *ts = spi_get_drvdata(spi);
	...
	ts->ext_wdata.time.rtc_sct.hour = cfg.rtc_cwhour;
	ts->ext_wdata.time.rtc_sct.min = cfg.rtc_cwmin;
	ts->ext_wdata.time.rtc_sct.sec = cfg.rtc_cwsec;
	ts->ext_wdata.time.rtc_sctcnt = cfg.rtc_cwmilli;

	TOUCH_I("%s : %02d:%02d:%02d.%03d\n", __func__,
		cfg.rtc_cwhour, cfg.rtc_cwmin, cfg.rtc_cwsec, cfg.rtc_cwmilli);

	ret = touch_device_func->etw_synctime(ts->spi_device, &ts->ext_wdata);
	if ( ret )
		TOUCH_E("%s fail\n", __func__);
...
static ssize_t show_extwatch_get_cfg(struct spi_device *spi, char *buf)
{
	TOUCH_I("%s \n", __func__);

	extwatch_dataload(spi);

	return 0;
}

static ssize_t extwatch_font_access_read(struct file *filp,
	struct kobject *ext_watch_kobj, struct bin_attribute *bin_attr,
	char *buf, loff_t off, size_t count)
{
	struct lge_touch_data *ts = container_of(ext_watch_kobj,
		struct lge_touch_data, ext_watch_kobj);
	ssize_t retval = -EFAULT;

	if (off == 0 && count == PAGE_SIZE) {
		touch_device_func->etw_font_dump(ts->spi_device, ts->ext_wdata.font_data);
	}
	...
	if (count == 0 && off + count >= ts->font_access_size) {
		...
		memcpy(buf, &ts->ext_wdata.font_data[off], count);
		retval = count;
	}
...
static ssize_t extwatch_font_access_write(struct file *filp, struct kobject *
ext_watch_kobj,
	struct bin_attribute *bin_attr, char *buf, loff_t off, size_t count)
{
	...
	if (count == 0 && off + count >= ts->font_access_size) {
		...
		memcpy(&ts->ext_wdata.font_data[off], buf, count);
		retval = count;

		if (off + count == FONT_DATA_SIZE) {
			touch_device_func->etw_font_download(ts->spi_device,
				ts->ext_wdata.font_data);
...
{
	ts->font_written_size = 0;
	ts->font_access_size = (64 * 1024);
	ts->ext_wdata.font_data = kzalloc(ts->font_access_size, GFP_KERNEL);
	if (ts->ext_wdata.font_data)
		TOUCH_I("%s font_buffer(%d KB) malloc\n", __func__,
			ts->font_access_size/1024);
	else {
		TOUCH_I("%s font_buffer(%d KB) malloc failed\n", __func__,
			ts->font_access_size/1024);
		return 1;
	}

	sysfs_bin_attr_init(&ts->font_access_attr);
	ts->font_access_attr.attr.name = "config_fontdata";
	ts->font_access_attr.attr.mode = S_IWUSR | S_IRUSR;
	ts->font_access_attr.read = extwatch_font_access_read;
	ts->font_access_attr.write = extwatch_font_access_write;
	ts->font_access_attr.size = ts->font_access_size;

	if (sysfs_create_bin_file(&ts->ext_watch_kobj, &ts->font_access_attr) < 0)
		TOUCH_I("Failed to create %s\n", ts->font_access_attr.attr.name);

	return 0;
}

static void extwatch_font_access_exit(struct lge_touch_data *ts)
{
	ts->font_written_size = 0;

	if (ts->ext_wdata.font_data) {
		//kfree(ts->ext_wdata.font_data);
		ts->ext_wdata.font_data = NULL;
	}
}
...
NULL, store_extwatch_fonteffect_config);
static LGE_TOUCH_ATTR(config_fontproperty, S_IRUGO | S_IWUSR,
NULL, store_extwatch_fontproperty_config);
static LGE_TOUCH_ATTR(config_fontposition, S_IRUGO | S_IWUSR,
NULL, store_extwatch_fontposition_config);
static LGE_TOUCH_ATTR(config_timesync, S_IRUGO | S_IWUSR,
NULL, store_extwatch_timesync_config);

static LGE_TOUCH_ATTR(current_time, S_IRUGO | S_IWUSR,
show_extwatch_current_time, NULL);
static LGE_TOUCH_ATTR(query_fontdata, S_IRUGO | S_IWUSR,
show_extwatch_fontdata_query, NULL);
static LGE_TOUCH_ATTR(query_fontposition, S_IRUGO | S_IWUSR,
show_extwatch_fontposition_query, NULL);
static LGE_TOUCH_ATTR(query_timesync, S_IRUGO | S_IWUSR,
show_extwatch_fonttime_query, NULL);
static LGE_TOUCH_ATTR(query_fontcolor, S_IRUGO | S_IWUSR,
show_extwatch_fontcolor_query, NULL);
static LGE_TOUCH_ATTR(query_fonteffect, S_IRUGO | S_IWUSR,
show_extwatch_fonteffect_query, NULL);
static LGE_TOUCH_ATTR(get_cfg, S_IRUGO | S_IWUSR,
show_extwatch_get_cfg, NULL);
...
static struct attribute *ext_watch_attribute_list[] = {
	...
	&lge_touch_attr_config_fontposition.attr,
	&lge_touch_attr_config_timesync.attr,
	&lge_touch_attr_current_time.attr,
	&lge_touch_attr_query_fontdata.attr,
	&lge_touch_attr_query_fontposition.attr,
	&lge_touch_attr_query_timesync.attr,
	&lge_touch_attr_query_fontcolor.attr,
	&lge_touch_attr_query_fonteffect.attr,
	&lge_touch_attr_get_cfg.attr,
	NULL,
};
		...
		struct attribute *attr, char *buf)
{
	...
	if (lge_touch_priv->show)
		ret = lge_touch_priv->show(ts->spi_device, buf);
		...
		const char *buf, size_t count)
{
	...
	if (lge_touch_priv->store)
		ret = lge_touch_priv->store(ts->spi_device, buf, count);

	return ret;
}

static ssize_t ext_wath_attr_show(struct kobject *ext_watch_kobj,
		struct attribute *attr, char *buf)
{
	struct lge_touch_data *ts = container_of(ext_watch_kobj,
			struct lge_touch_data, ext_watch_kobj);
	struct lge_touch_attribute *ext_watch_priv =
		container_of(attr, struct lge_touch_attribute, attr);
	ssize_t ret = 0;

	if (ext_watch_priv->show)
		ret = ext_watch_priv->show(ts->spi_device, buf);

	return ret;
}

static ssize_t ext_wath_attr_store(struct kobject *ext_watch_kobj,
		struct attribute *attr,
		const char *buf, size_t count)
{
	struct lge_touch_data *ts = container_of(ext_watch_kobj,
	...
	if (ext_watch_priv->store)
		ret = ext_watch_priv->store(ts->spi_device, buf, count);
...
static const struct sysfs_ops ext_watch_sysfs_ops = {
	.show	= ext_wath_attr_show,
	.store	= ext_wath_attr_store,
};
...
static struct touch_platform_data *get_dts_data(struct device *dev)
{
	...
	p_data->int_pin = of_get_named_gpio_flags(np,
			"irq-gpio", 0, NULL);
	p_data->cs_pin = of_get_named_gpio_flags(np,
			"cs-gpio", 0, NULL);
...
{
	if (spi->dev.of_node)
		ASSIGN(*p_data = get_dts_data(&spi->dev), error);
	else
		ASSIGN(*p_data = (struct touch_platform_data *)
				spi->dev.platform_data, error);
...
static int add_filter_func(struct lge_touch_data *ts)
{
	...
	ADD_NEW_FILTER(ts->spi_device->dev, bouncing_filter, ts->filter_head.list);
	ADD_NEW_FILTER(ts->spi_device->dev, grip_filter, ts->filter_head.list);
	ADD_NEW_FILTER(ts->spi_device->dev, accuracy_filter, ts->filter_head.list);
	ADD_NEW_FILTER(ts->spi_device->dev, jitter_filter, ts->filter_head.list);
	ADD_NEW_FILTER(ts->spi_device->dev, quickcover_filter,
			ts->filter_head.list);
...
static void remove_filter_func(struct lge_touch_data *ts)
{
	...
	list_for_each_safe(pos, n, &ts->filter_head.list) {
		tmp = list_entry(pos, struct filter_func, list);
		list_del(pos);
		devm_kfree(&ts->spi_device->dev, tmp);
	}
}
...
static int sysfs_register(struct lge_touch_data *ts)
{
	...
	DO_SAFE(ret = kobject_init_and_add(&ts->ext_watch_kobj,
				&ext_watch_kobj_type,
				ts->input_dev->dev.kobj.parent->parent,
				"%s", LGE_EXT_WATCH_NAME),
			err_kobject_init_and_add);

	ret = sysfs_create_group(&ts->ext_watch_kobj,
			&ext_watch_attribute_group);

	extwatch_font_access_init(ts);
	return ret;
...
static int touch_suspend(struct device *dev)
{
	...
	atomic_set(&ts->state.uevent, UEVENT_IDLE);
	mutex_lock(&ts->pdata->thread_lock);
	touch_device_func->suspend(ts->spi_device);
	...
	if ((quick_cover_status == QUICKCOVER_CLOSE)
		...
		touch_device_func->lpwg(ts->spi_device,
				LPWG_ACTIVE_AREA_X1,
...
static int touch_resume(struct device *dev)
{
	...
	if (window_crack_check == CRACK) {
		...
		TOUCH_I("%s, %d : window craked - send uvent\n",
				__func__, __LINE__);
		send_uevent(&ts->spi_device->dev, window_crack);
	}
	...
	if (atomic_read(&ts->state.upgrade) != UPGRADE_START) {
		touch_device_func->resume(ts->spi_device);
	} else
		TOUCH_D(DEBUG_BASE_INFO,
...
static void touch_early_suspend(struct early_suspend *h)
{
	...
	touch_suspend(&ts->spi_device->dev);
}
...
static void touch_late_resume(struct early_suspend *h)
{
	...
	touch_resume(&ts->spi_device->dev);
}
		...
		unsigned long event, void *data)
{
	...
	case LCD_EVENT_TOUCH_SLEEP_STATUS:
		...
		case SET_SLEEP_STATUS:
			...
			touch_device_func->power(ts->spi_device,
					POWER_SLEEP_STATUS);
			break;
		case SET_NO_SLEEP:
		...
		break;
	case LCD_EVENT_HW_RESET:
		TOUCH_I("LCD_EVENT_HW_RESET!\n");
		interrupt_control(ts, INTERRUPT_DISABLE);
		atomic_set(&ts->state.need_font_downoload, DOWNLOAD_NEED);
		break;
...
void touch_sleep_status(struct spi_device *spi, int data)
{
	struct lge_touch_data *ts = spi_get_drvdata(spi);
...
void touch_swipe_status(struct spi_device *spi, int mode)
{
	struct lge_touch_data *ts = spi_get_drvdata(spi);
	if ((ts->pdata->role->use_lcd_notifier_callback))
...
static int touch_probe(struct spi_device *spi)
{
	...
	ASSIGN(ts = devm_kzalloc(&spi->dev,
				sizeof(struct lge_touch_data), GFP_KERNEL),
			error);

	DO_SAFE(get_platform_data(&ts->pdata, spi), error);

	spi_set_drvdata(spi, ts);
	ASSIGN(ts->spi_device = spi, error);
	...
	if (ts->pdata->cs_pin > 0) {
		DO_SAFE(ret = gpio_request(ts->pdata->cs_pin,
					"touch_cs"), error);
		gpio_direction_output(ts->pdata->cs_pin, 0);
	}

	memset(&ts->state, 0, sizeof(struct state_info));
	state = &ts->state;
	spi_device_only_for_update_status = spi;
...
#if 0
	/* Get iface_clk info */
        ts->iface_clk = clk_get(&ts->spi_device->dev, "iface_clk");
  	if (IS_ERR(ts->iface_clk)) {
		TOUCH_E("Failed to get iface_clk\n");
    	return PTR_ERR(ts->iface_clk);
  	}

  	/* Get core_clk info */
  	ts->core_clk = clk_get(&ts->spi_device->dev, "core_clk");
  	if (IS_ERR(ts->core_clk)) {
		TOUCH_E("Failed to get core_clk\n");
    	return PTR_ERR(ts->core_clk);
 	 }
#endif	
	spi->bits_per_word = 8;
	spi->mode = SPI_MODE_0;
	spi->max_speed_hz = SPI_MAX_CLOCK_SPEED;
	DO_SAFE(spi_setup(spi), error);
#if 0
	/* Enable the spi clocks */
	clk_set_rate(ts->core_clk, spi->max_speed_hz);

	ret = clk_prepare_enable(ts->core_clk);
    	if (ret) {
		TOUCH_E("Error enabling core clk\n");
    	}

    	ret = clk_prepare_enable(ts->iface_clk);

    	if (ret) {
		TOUCH_E("Error enabling iface clk\n");
    	}
#endif
	irq = spi->irq;
	cs = spi->chip_select;
	...
	TOUCH_I("%s : CS[%x] CPHA[%x] CPOL[%x] CS_HIGH[%x] Clock[%d Mhz] \n",
			__func__, cs, cpha, cpol, cs_high, max_speed/1000000);

	DO_IF(touch_device_func->probe(ts->spi_device, ts->pdata,
				&ts->state) != 0,
			error);
	...
	INIT_DELAYED_WORK(&ts->work_font_download, font_download_func);
	...
	if (touch_ic_init(ts, 0) < 0) {
		touch_device_func->remove(ts->spi_device);
		ret = -ENOMEM;
		goto err_ic_init;
	}

	DO_SAFE(ret = request_threaded_irq(ts->spi_device->irq,
...
static int touch_remove(struct spi_device *spi)
{
	...
	input_unregister_device(ts->input_dev);
	devm_kfree(&ts->spi_device->dev, ts);

	return 0;
}

static void touch_shutdown(struct spi_device *spi)
{
	struct lge_touch_data *ts = spi_get_drvdata(spi);

	TOUCH_TRACE();

	touch_device_func->shutdown(ts->spi_device);
	extwatch_font_access_exit(ts);
}
...
static int touch_pm_resume(struct device *dev)
{
	...
	if (atomic_read(&ts->state.pm) == PM_SUSPEND_IRQ) {
		struct irq_desc *desc = irq_to_desc(ts->spi_device->irq);

		if (desc == NULL) {
			TOUCH_D(DEBUG_BASE_INFO,
					"Null Pointer from irq_to_desc(ts->spi_device->irq)\n"
					);
			return -ENOMEM;
		}
		...
		irq_set_pending(ts->spi_device->irq);
		check_irq_resend(desc, ts->spi_device->irq);
...
static struct spi_device_id lge_ts_id[] = {
	{LGE_TOUCH_NAME, 0 },
};


static struct spi_driver lge_touch_driver = {
	.probe   = touch_probe,
	.remove	 = touch_remove,
		...
		struct of_device_id *match_table)
{
	...
	DO_SAFE(ret = spi_register_driver(&lge_touch_driver), err_spi_add_driver);
	return 0;

err_spi_add_driver:
	destroy_workqueue(touch_wq);
	TOUCH_D(DEBUG_BASE_INFO, "err_spi_add_driver!!!\n");
	TOUCH_TRACE();
err_create_workqueue:
	return ret;
}
...
void touch_driver_unregister(void)
{
	...
	spi_unregister_driver(&lge_touch_driver);
